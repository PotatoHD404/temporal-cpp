...
description: Сборка temporal-cpp на macOS, Linux или любой платформе через Conan, установка и подключение из другого CMake-проекта через find_package.
---
...
| `TEMPORAL_INSTALL` | `ON` | Генерировать правила `install()` и config-пакет `find_package(temporal-cpp)`. |
...
потребитель, используемый как CI-смоук-тест установки.)
