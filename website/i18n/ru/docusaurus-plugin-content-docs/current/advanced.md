...
Отмена детерминирована — при реплее воркфлоу воспроизводит команду `CancelTimer`. Отмена активностей и дочерних воркфлоу, а также автоматическое распространение отмены с уровня воркфлоу ещё не реализованы (отмену на уровне воркфлоу можно только наблюдать через `ctx.IsCancelled()`).
