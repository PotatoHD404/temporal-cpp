...
Когда воркфлоу отменяют, `AwaitCancellation` завершается, селектор выбирает эту ветку, и воркфлоу отменяет свою операцию и быстро завершается. Тот же паттерн отменяет **активность**: запустите гонку `ctx.ExecuteActivity<…>()` против `AwaitCancellation`, при отмене вызовите `act.Cancel()` (он отправляет `RequestCancelActivityTask`), и активность увидит запрос через `activity::Context::IsCancelled()` в своём цикле heartbeat и вернётся. Отмена дочерних воркфлоу и автоматическое распространение (отмена всех выполняющихся операций без ручной обвязки) — оставшиеся части.
